...
ops.uniaxialMaterial('Steel01', 1, 10.0, 1e5, 0.01)
ops.section('Uniaxial', 1, 1, 'Mz')

# ops.element('elasticBeamColumn', 11, 11, 21, 0.1, 1e5, 0.01, 1)
# ops.element('elasticBeamColumn', 12, 12, 22, 0.1, 1e5, 0.01, 1)

ops.element('beamWithHinges', 11, 11, 21, 1, 0.1, 1, 0.1, 1e5, 0.01, 1)
ops.element('beamWithHinges', 12, 12, 22, 1, 0.1, 1, 0.1, 1e5, 0.01, 1)
...
ops.recorder('Element', '-file', 'force.out', '-time', '-ele', 21, 22, 'force')
ops.analyze(20)
ops.wipe()
...
plt.figure()
plt.plot(disp[:, 2] - disp[:, 1], force[:, 4] + force[:, 10], 'o-')
