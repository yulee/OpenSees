...
ops.uniaxialMaterial('Steel01', 2, 4e2, 2e5, 0.01)
...
force2 = np.loadtxt('force1.out')
plt.figure()
plt.plot(disp[:, 1], force1[:, 4] + force1[:, 10], 'o-')
plt.plot(disp[:, 2] - disp[:, 1], force2[:, 4] + force2[:, 10], 'o-')
# plt.plot(disp[:, 2] - disp[:, 1], force[:, 4] + force[:, 10], 'o-')
# plt.plot(disp[:, 2] - disp[:, 1], force[:, 10], 'o-')
